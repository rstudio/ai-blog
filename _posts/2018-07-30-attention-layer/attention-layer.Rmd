---
title: "Looking for attention? Here it is"
description: |
  No way one could compete with that title: "Attention is all you need". Also, in our text attention is not all you need: We're showing how to augment a classic Keras seq2seq model with an attention mechanism. This text is based on the   excellent "Neural Machine Translation" tutorial featuring prominently among the TensorFlow eager execution docs.  We're explaining how to port this to R. And of course, we need to differentiate ourselves by using a different dataset.
author:
  - name: Sigrid Keydana
    affiliation: RStudio
...
As of this writing, it is not difficult to find sample code that shows how to perform sequence to sequence translation using Keras. However, a few years ago already it has been established that depending on the task, incorporating an attention mechanism significantly improves performance. ^[First and foremost, this was the case for neural machine translation (see @DBLP:journals/corr/BahdanauCB14 and @DBLP:journals/corr/LuongPM15 for prominent work). But other areas performing sequence to sequence translation were profiting from incorporating an attention mechanism, too: E.g., @DBLP:journals/corr/XuBKCCSZB15 applied attention to image captioning, and @DBLP:journals/corr/VinyalsKKPSH14, to parsing.] Ideally, using Keras, we'd just have an attention layer managing this for us. Unfortunately, as can be seen googling for code snippets and blog posts, implementing attention in pure Keras is not that straightforward.

Consequently, until a short time ago, the best thing to do seemed to be translating the [TensorFlow Neural Machine Translation Tutorial](https://github.com/tensorflow/nmt) to R TensorFlow. Then, [TensorFlow eager execution](https://www.tensorflow.org/guide/eager) happened, and turned out a game changer for a number of things that used to be difficult (last not least, debugging). With eager execution, operations are executed immediately, as opposed to of building a graph to be evaluated later. This means we can immediately inspect the values in our tensors - and it also means we can imperatively code loops to perform interleavings of sorts that earlier were more challenging to accomplish.
...
Still from the TensorFlow stack, we are going to use [tfdatasets](https://tensorflow.rstudio.com/tools/tfdatasets/articles/introduction.html). So we end up with the following libraries needed for this example:
...
We have specified our GRU to return sequences as well as the state. Us asking for the state means we'll get back a list of tensors: the output, and the last state(s) - a single last state in this case as we're using GRU. That state itself will be of shape `(batch_size, gru_units)`.
Us asking for sequences means the output will be of shape `(batch_size, max_length_input, gru_units)`. So that's that. We bundle output and last state in a list and pass it to the calling code.
...
Although various attention mechanisms exist, the basic procedure often goes like this. ^[In our description, we're closely following @DBLP:journals/corr/LuongPM15, in accordance with the colaboratory notebook on NMT.]
...
The score function can take different shapes; the following is commonly referred to as _Bahdanau style_ (additive) attention ^[When referring to this as _Bahdanau style attention_, we - like others - do not imply exact agreement with the formulae in @DBLP:journals/corr/BahdanauCB14. It is about the general way encoder and decoder hidden states are combined - additively (or by concatenation) or multiplicatively.]
  ...
  function(object,
    ...
    keras_model_custom(name = name, function(model) {
      ...
      function(inputs, mask = NULL) {
        ...
        score <- model$V(k_tanh(model$W1(encoder_output) + model$W2(hidden_with_time_axis)))
...
We still have to take care of the third source of information: the input. After it has been passed through the embedding layer, it's shape is `(batch_size, 1, embedding_dim)`. Here, the second axis is of dimension 1 as we're forecasting a single token at a time.
...
Now, we create the encoder and decoder objects - it's tempting to call them layers, but technically both inherit from `RModel`. 
...
This `variables` slot, by the way, does not (as of this writing) exist in "Keras Keras", which is why we have to resort to the TensorFlow implementation.
...
The [complete code for this post](tbd) will do it like this, however here we're arranging the steps in a more didactical order...
...
Now, how about subjecting our network to a little CT scan? Since we're collecting the attention weights, we can visualize what part of the source text the decoder is _attending to_ at every timestep.
...
Last not least, no one said attention need be useful only in the context of machine translation. Out there, a plenty of sequence prediction (time series) problems are waiting to be checked for its potential usefulness...

```
[1] "Input: <start> I did that easily . <stop>"              
[2] "Input: <start> Ik heb dat zonder moeite gedaan . <stop>"
[1] "Predicted translation: <start> Ik heb dat zonder moeite gedaan . <stop>
```