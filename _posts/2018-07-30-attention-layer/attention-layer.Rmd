...
You should also be sure that you are running the very latest version of TensorFlow (v1.9), which you can install as follows:
...
There are additional requirements for using TensorFlow eager execution. First, we need to call `tfe_enable_eager_execution()` right at the beginning of the program. Second, we need to use implementation of Keras included in TensorFlow, rather than the base Keras implementaiton. This is because at a later point, we are going to access `model$variables` which at this point does not exist in base Keras.
