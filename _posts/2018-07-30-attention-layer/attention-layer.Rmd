...
As of this writing, it is not difficult to find sample code (be it R or Python) that shows how to perform sequence to sequence translation. However, it has been established already a few years ago ^[footnote] that depending on the task, incorporating an attention mechanism significantly improves performance. Ideally, using Keras, we'd just have an attention layer handling this for us. Unfortunately, as can be seen googling for code snippets and blog posts, implementing attention in pure Keras is not that straightforward.
...
Now let's look at how the attention decoder implements the above logic. We will be following the colab notebook in presenting a slight simplification of the above logic, which will not prevent the code from successfully translating our example sentences. Feel free to experiment with more intricate logic if you're curious!
  ...
  function(object,
    ...
    keras_model_custom(name = name, function(model) {
      ...
      function(inputs, mask = NULL) {
        ...
        score <-
          model$V(k_tanh(model$W1(encoder_output) + model$W2(hidden_with_time_axis)))
...
Now first we need to calculate the score, which basically means addition of two matrix multiplications.
...
After applying the _tanh_ and the fully connected layer to the result of the addition, `score` will be of shape `(batch_size, max_length_input, 1). Now the next step calculates the softmax, to get the _attention weights_.
...
Now we compute the context vector, as a weighted average of encoder hidden states. Its shape now is `(batch_size, gru_units)`. Note that like with the softmax operation above, we sum over the second axis, which corresponds to the number of timesteps in the input received from the encoder.

Now we still have to take care of the third source of information: the input. After it has been passed through the embedding layer, it's shape is `(batch_size, 1, embedding_dim)`. Here, the second axis is of dimension 1 as we're forecasting a single token at a time.
...
Remains to return everything we're interested in: the output (to be used for forecasting), the last GRU hidden state (to be passed back in to the decoder), and the attention_weights for this batch (for plotting). And that's that! 
