...
description: |
  Image captioning is a challenging task at the vision-language interface. Continuing our mini-series on combining Keras with TensorFlow eager execution, we show a way to incorporate an attention mechanism that allows the network to concentrate on image features relevant to the current state of text generation.
author:
  - name: Sigrid Keydana
    affiliation: RStudio
...
In image captioning, an algorithm is given an image and tasked to produce a sensible caption. It is a challenging task for several reasons, not the least being that it involves a notion of _saliency_ or _relevance_. This is why recent deep learning approaches mostly include one or the other attention mechanism (sometimes even more than one) to help focusing on relevant image features. 
...
No need to copy-paste any code snippets - you'll find the complete code (in order necessary for execution) [here](https://github.com/rstudio/keras/blob/master/vignettes/examples/eager-image-captioning.R).
...
![From MS-COCO 2014](images/COCO_train2014_000000510592.jpg){.external width=50%}
...
![From MS-COCO 2014](images/COCO_train2014_000000118167.jpg){.external width=50%}
...
_InceptionV3_ being a "big model", where every pass through the model takes its time, we want to precompute features in advance and store them on disk. 
...
rnn_decoder <- function(embedding_dim, gru_units, vocab_size, name = NULL) {
  ...
  keras_model_custom(name = name, function(self) {
    ...
    self$embedding <- layer_embedding(input_dim = vocab_size, output_dim = embedding_dim)
  ...
  for (epoch in seq_len(num_epochs)) {
    ...
    until_out_of_range({
      ...
      dec_input <- k_expand_dims(
        rep(list(word_index_df[word_index_df$word == "<start>", "index"]), batch_size)
      )
...
Just like in the translation case, it's interesting to look at model performance already during training. The companion code has that functionality integrated (and required functions defined in the correct order), so you can watch model progress for yourself. 

The basic function here is `get_caption`: It gets passed the path to an image, loads it, obtains its features from Inception V3, and then asks the encoder-decoder model to generate a caption. If at any point the model produces the `end` symbol, we stop early. Otherwise, we may generate a sentence extending up till our predefined maximum length.
...
This certainly tells that the network has been able to generalize over - let's not call them concepts, but mappings between visual and textual entities, perhaps. It's true that it will have seen some of these images before, because images come with several captions. You could be more strict setting up your training and validation sets - but here, we don't really care about objective performance scores and so, it does not really matter.
