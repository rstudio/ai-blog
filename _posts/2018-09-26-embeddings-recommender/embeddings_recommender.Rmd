...
Here's a minimal [custom model](https://tensorflow.rstudio.com/keras/articles/custom_models.html) that does exactly this:^[Custom models are a recent Keras feature that allow for a flexible definition of the forward pass. While the current use case does does not require using a custom model, it nicely illustrates how the network's logic can quickly be grasped by looking at the _call_ method.]
