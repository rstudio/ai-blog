---
title: "Road to object detection 1: Combining classification and localization"
description: >
...
How should we pass our data to Keras? We could simple use Keras' `image_data_generator`, but given we will need custom generators soon, we'll build a simple one ourselves here already.
This one simply delivers images as well as the corresponding targets in a stream. Note how the targets are not one-hot-encoded, but integers - using `sparse_categorical_crossentropy` as a loss function enables this convenience.
...
for (i in 1:8) {
  preds <-
    model %>% predict(
      load_and_preprocess_image(train_1_8[i, "file_name"], target_height, target_width),
      batch_size = 1
  )
