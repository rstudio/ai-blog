...
- the loss function designed to minimize aleatoric uncertainty,
- and the ways we can obtain both uncertainties at test time.
...
# R6 wrapper class, a subclass of KerasWrapper, just like we can subclass KerasLayer
ConcreteDropout <- R6::R6Class("ConcreteDropout",
  ...
  public = list(
    ...
    build = function(input_shape) {
      ...
      kernel_regularizer <- self$weight_regularizer * k_sum(k_square(weight)) / (1 - self$p)
      
      dropout_regularizer <- self$p * k_log(self$p)
      dropout_regularizer <- dropout_regularizer +  (1 - self$p) * k_log(1 - self$p)
      dropout_regularizer <- dropout_regularizer * self$dropout_regularizer * k_cast(input_dim, k_floatx())
    ...
    concrete_dropout = function(x) {
      ...
      drop_prob <- k_log(self$p + eps) - k_log(1 - self$p + eps) + k_log(unif_noise + eps) - k_log(1 - unif_noise + eps)
...
# and just like with custom layers, we have an accompanying instantiator function
...
c(X_val, Y_val) %<-% list(X[(n_train + 1):(n_train + n_val)], Y[(n_train + 1):(n_train + n_val)])
...
df <- data.frame(
  ...
  u_overall_lower = predictive_mean - sqrt(epistemic_uncertainty) - sqrt(aleatoric_uncertainty),
  u_overall_upper = predictive_mean + sqrt(epistemic_uncertainty) + sqrt(aleatoric_uncertainty)
)
```
...
```{r}
ggplot(df, aes(x, y_pred)) + geom_point() + geom_ribbon(aes(ymin = e_u_lower, ymax = e_u_upper), alpha = 0.3)
```
