...
$$p(\mathbf{x}) =  p(\mathbf{z}) \frac{vol(d\mathbf{z})}{vol(d\mathbf{x})}$$
...
$$p(\mathbf{x}) = p(\mathbf{z}) \ \bigg|det\frac{\partial{\mathbf{z}}}{\partial{\mathbf{x}}}\bigg|$$

In practice, we work with log probabilities, so

$$log \ p(\mathbf{x}) = log \ p(\mathbf{z}) - log \ \bigg|det\frac{\partial{\mathbf{z}}}{\partial{\mathbf{x}}}\bigg| $$
...
We can verify the cumulative probability stays the same using `tfd_transformed()`:

```{r}
d_t <- tfd_transformed(distribution = d_n, bijector = b)
...
With eager execution, the variables have to be used inside the loss function, so that is where we define the bijectors. Our little flow now is a `tfb_chain` of bijectors, and we wrap it in a _TransformedDistribution_ (`tfd_transformed`) that links source and target distributions.  
...
loss <- function() {
 ...
 dist <- tfd_transformed(distribution = base_dist,
                          bijector = flow)
