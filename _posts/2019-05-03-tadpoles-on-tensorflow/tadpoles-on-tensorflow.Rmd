...
So before we jump into the technicalities, -- yes, that's what this post is too: _Un hommage à_ McElreath who wrote one of most intriguing books on Bayesian (or should we just say - scientific?) modeling we're aware of. If you haven't read _Statistical Rethinking_, and are interested in modeling, you might definitely want to check it out. In this post, we're not going to try to re-tell the story: Our clear focus will instead be a demonstration of how to do MCMC with [tfprobability](https://rstudio.github.io/tfprobability/).

Concretely, this post has two parts. The first is a quick overview of how to use `tfd_joint_sequential_distribution` to construct a model, and then sample from it using Hamiltonian Monte Carlo. This part could be consulted for quick code look-up, or as a frugal template of the whole process.
...
As with every TFP distribution, a quick functionality check is sampling from the model:
...
You might be wondering why we are not using eager execution, something we normally like to do.
The reason is that as of today, the sampling code must be executed on the graph in order to be run in parallel -- which definitely is something we'll want to do when with a set of Markov chains. In Python, this can be achieved by annotating the sampling function with `@tf.function`, a construct that as of this writing is not available from R.
...
plot_posteriors <- function(sample_array, num_params) {
  plots <- vector(mode = "list", length = num_params)
  for (i in 1:num_params) {
    p <- plot_posterior(sample_array[ , , i] %>% as.matrix())
    plots[[i]] <- p
  }
