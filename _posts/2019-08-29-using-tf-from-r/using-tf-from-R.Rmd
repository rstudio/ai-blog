...
Which computer language is most closely associated with _TensorFlow_? While on the _TensorFlow for R blog_, we would of course like the answer to be _R_, chances are it is Python (though TensorFlow has official bindings for C, Java, and Go as well).
...
The short answer is, you have `keras`, `tensorflow` and `reticulate` installed which taken together, create and manage Python objects for you and let you operate on them on a high level. 
This post first elaborates a bit on the short answer. We then look deeper into what happens in the background.
...
This is because `keras` defines an actual Python class, `RCallback`, and maps your R6 class'es methods to it.
...
To find out more about what is going on, debugging on the R level won't get us far. We start `gdb` so we can set breakpoints in C++ functions: ^[For a very nice introduction to debugging R with a debugger like `gdb` or `lldb`, see [Kevin Ushey's "Debugging with LLDB"](http://kevinushey.github.io/blog/2015/04/13/debugging-with-lldb/.)]
...
While strictly, Python is a language specification, its reference implementation is CPython, built of C and Python. In CPython, every Python object (including integers or other numeric types) is a `PyObject`. `PyObject`s are allocated through and operated on using pointers; most C API functions return a pointer to one, `PyObject *`.

So this is what we expect to work with, from R. What then is `PyObjectRef` doing in `py_ref_to_r`?
`PyObjectRef` is not part of the C API, it is part of the functionality introduced by `reticulate` to manage Python objects. Its main purpose is to make sure the Python object is automatically cleaned up when the R object (an `Rcpp::Environment`) goes out of scope. It also stores the "convert" feature of the Python object, used to determine whether that object should be converted to R automatically.
...
and -- but wait. Didn't `py_ref_to_r_with_convert` pass it a `PyObjectRef`? So how come it receives a `PyObject` instead? This is because `PyObjectRef` inherits from `Rcpp::Environment`, and its address-of operator is used to extract the Python object from the `Environment`:
...
So when `py_to_r`, inspecting it's argument's type, sees it is a Python callable (wrapped in a `PyObjectRef`, the `reticulate`-specific subclass of `Rcpp::Environment` we talked about above), it wraps it (the `PyObjectRef`) in an R function, using Rcpp:
...
This assigns `1` to a variable `a` in the python main module. A prerequisite for the assignment to happen is getting that `1` converted to Python. (We're using the simplest possible example, obviously; but you can imagine this getting a lot more complex if the object isn't a simple number).
...
R is a concise and elegant language, but to a high degree its power comes from its libraries. Hearing "libraries", we think of the `tidyverse`, of course; but there's also the packages that allow you to call into, and interact with, the outside world, such as deep learning frameworks or distributed processing engines. In this post, it was a special pleasure to focus on a central building block that makes much of this action possible: `reticulate`. 
