...
`python_object_finalize` is interesting, as it tells us something crucial about Python -- about CPython, to be precise: CPython does not have garbage collection; instead, it uses reference counting, placing on the user the burden of correctly incrementing and decrementing references according to language semantics.
...
The above _finalizer_ mechanism thus frees `reticulate` code of having to decrement reference count on every single object.
