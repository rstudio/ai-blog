---
title: "TensorFlow 2.0 is there - what changes for R users?"
description: > 
  TensorFlow 2.0 finally having been released last week, as R users we have two kinds of questions. First, will my keras code still run? And second, what is it that changes? In this post, we answer both and then, give a tour of exciting new developments in the r-tensorflow ecosystem.
author:
  - name: Sigrid Keydana
    affiliation: RStudio
...
The waiting is over -- TensorFlow 2.0 (TF 2) is now officially there! What does this mean for us, users of R packages `keras` and/or `tensorflow`, which, as we know, [rely on the Python TensorFlow backend](https://blogs.rstudio.com/tensorflow/posts/2019-08-29-using-tf-from-r/)?

Before we go into details and explanations, here is an _all-clear_, for the concerned user who fears their `keras` code might get obsolete (it won't).
...
- And perhaps most interestingly: Take a look at what is going on, in the `r-tensorflow` ecosystem, around new functionality brought about, or related to somehow, the advent of TF 2.
...
This is nice, but when creating the model, we'll still have to define all those layers, right? (Which should be pretty cumbersome, having to figure out all the right dimensions...)
...
As a last item on the topics of preprocessing and feature engineering, let's look at a much-promising thing to come in what we hope is the near future.
