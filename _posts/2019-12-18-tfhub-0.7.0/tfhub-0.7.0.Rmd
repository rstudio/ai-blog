...
> **Note**: If you are using the most recent CRAN release of reticulate (1.14.0), tfhub will be automatically installed when you `library` it for the first time.

## Simple example

The essencial function of tfhub is `layer_hub` which works just like a [keras](https://github.com/rstudio/keras) but allows you to load a complete pre-trained Deep Learning model.
...
This will download the MobileNet model pre-trained in the ImageNet dataset. tfhub models are cached locally and don't need to be downloaded the next time you use the same model.

You can now use `layer_mobilenet` as an usual Keras layer. For example you can define a model:
...
This model can now be used to create predict Imagenet labels for an image. For example, let's see the results for the famous Grace Hopper's photo:
...
imagenet_decode_predictions(pred[,-1,drop=FALSE])
```
...
All possible models can be found in the TensorFlow hub [website](https://tfhub.dev).

![TensorFlow Hub](images/tfhub.png)

You can find more examples of `layer_hub` usage in the following articles of the TensorFlow for R website:
...
## tfdatasets integration

You can also use tfhub with the new [`feature_spec` API](https://tensorflow.rstudio.com/guide/tfdatasets/feature_spec/) implemented in tfdatasets. You can see a complete example [here](https://tensorflow.rstudio.com/guide/tfhub/examples/feature_column/).
