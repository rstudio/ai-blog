...
As a quick recap, the pins package allows you to cache, discover and share resources. You can use `pins` in a wide range of situations, from downloading a dataset from a URL, to creating complex automation workflows, learn more about at [pins.rstudio.com](https://pins.rstudio.com). You can also use `pins` in combination with TensorFlow and Keras; for instance, you can use the [cloudml](https://tensorflow.rstudio.com/tools/cloudml) to train models in cloud GPUs, but rather than manually copying files into the GPU instance, you can use pins directly from R.
...
To illustrate the new versioning functionality, lets start by downloading and caching a remote dataset with pins. For this example, we will download the weather in London, this happens to be in JSON format and requires `jsonlite` to be parsed:
...
One advantage of using `pins` is that, even if the URL or your internet connecetion becomes unavaiable, the above code will still work.
...
```r
jsonlite::read_json(pin(weather_url, "weather", signature = "624cca260666c6f090b93c37fd76878e3a12a79b"))
```

If the remote dataset changes, `pin()` will fail and you can take the appropriate steps to accept the changes by updating the signature or properly updating your data code, if needed. The previous example is useful as a way of detecting version changes, but we might also want to retrieve specific versions even when the dataset changes.

`pins 0.4` allows you to display and retrieve versions from services like GitHub, Kaggle and RStudio Connect, and even in boards that don't support versioning natively, you can opt-in by registering a board with `versions = TRUE`.
...
You can follow similar steps for RStudio and Kaggle boards, even for existing pins! Other boards like Amazon S3, Google Cloud, Digital Ocean and Microsoft Azure, require you explicitly enable versioning when registiring your boards.

# DigitalOcean

Yo try out the new [DigitalOcean Spaces board](http://pins.rstudio.com/articles/boards-dospace.html), first you will have to register this board and enable versioning by setting `versions` to `TRUE`:
...
To learn more abouut versioning visit the [Versioning Article](http://pins.rstudio.com/articles/advanced-versions.html), for DigitalOceaen use the [DigitalOcean Article](http://pins.rstudio.com/articles/boards-dospace.html), to cactch up with previous releases:
