...
- Improved capabilities to work with Spark SQL structs and arrays of Spark SQL structs in Spark 2.4 or above
   * `sdf_collect` can now collect them into equivalent lists or named lists in R
   * `dplyr` grammar can be applied to successfully query nested attributes within such datums
...
## Better support for Spark SQL structs
...
# Let's say we have a product catalog of computers and each computer has some attributes
computers_df <- tibble(
  id = seq(1, 2),
  attributes = list(
    # computer 1
    list(
      # processor of computer 1
...
# and we have connected to Spark 2.4 or above
sc <- spark_connect(master = "local", version = "2.4")

```

For this example, let's pretend we are querying structurally complex data that exists in Spark only and none of the columns has been deserialized into its equivalent in R yet. So, we shall copy `computers_df` into a Spark dataframe named `computers_sdf`, delete `computers_df`, and only query `computers_sdf` from now on.

```{r echo=TRUE, eval=FALSE}
computers_sdf <- sdf_copy_to(sc, computers_df, overwrite = TRUE)
rm(computers_df)
```

A typical `dplyr` use case involving `computers_sdf` would be the following:

```{r echo=TRUE, eval=FALSE}
high_freq_computers <- computers_sdf %>%
...
```
 ...
 where `high_freq_computers$attributes` is
