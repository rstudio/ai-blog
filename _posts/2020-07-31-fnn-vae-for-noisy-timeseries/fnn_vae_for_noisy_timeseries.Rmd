...
This post did not quite end up the way I'd imagined. A quick follow-up on the recent [Time series prediction with
FNN-LSTM](https://blogs.rstudio.com/ai/posts/2020-07-20-fnn-lstm/), it should have shown how *noisy time series* (so common in
...
Firstly, FNN-VAE, while on par, performance-wise, with FNN-LSTM, it is far superior in that other meaning of "performance":
Training goes a *lot* faster for FNN-VAE.

And secondly, while we don't see much difference between FNN-LSTM and FNN-VAE, we *do* see a clear impact of using FNN loss:
Especially for the VAE architecture, but for the LSTM one as well, adding in FNN loss strongly reduces mean squared error with
respect to the underlying (denoised) series. This is of particular interest with VAE, as it comes with a regularizer
out-of-the-box -- namely, Kullback-Leibler (KL) divergence.

Of course, we don't claim that similar results will always be obtained on other noisy series; and neither did we tune any of
...
how to use it for prediction (as opposed to reconstructing an attractor). "It", the technique, meant augmenting an LSTM
autoencoder with FNN loss. The results were stunning: In multi-step prediction (12-120 steps, with that number varying by
...
already hinted at above, it is completely thinkable that the "variational" part is not even so important here -- that a
convolutional autoencoder with just MSE loss would have performed just as well on those data. In fact, to find out, it's
enough to just remove the call to `reparameterize()` and multiply the KL component of the loss by 0. (We leave this to the
interested reader, to keep the post at reasonable length.)

One last contextual thing, in case you haven't read the two previous posts and would like to jump in here directly. We're
...
variational to these models per se; they really just are an encoder and a decoder, respectively. Metamorphosis into a VAE will
...
Here you can compare effects of adding no (left), standard deviation-1 (middle), and standard deviation-2.5 Gaussian noise:
...
Seeing how all models performed bravourously on the original deterministic series, a noise level of 1 can almost be treated as
...
Between noise levels 1.5 and 2, something changed, or became noticeable from visual inspection. Let's directly jump to the
...
Both LSTM and VAE, but the latter to an even higher degree, get "distracted" a bit too much by the noise, leading to cases
...
Put differently, it mostly is a function of noise level.
...
So, if we want to predict *signal plus noise* (left), it doesn't really matter whether we use FNN or not. But if we want to
