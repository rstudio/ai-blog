...
- Plotting the result shows the non-outlier data points being scaled to values that still more or less form a bell-shaped distribution centered around $0$, as expected, so the scaling is robust against influences of the outliers:
...
- From the plots one can observe while both of them result in some bell-shaped distributions, the former is centered around $0$, correctly indicating the average among all non-outlier values, while the latter is clearly not centered around $0$ as its center has been noticeably shifted by the $10$ outlier values.
