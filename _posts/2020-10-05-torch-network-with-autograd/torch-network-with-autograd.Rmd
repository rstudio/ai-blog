...
description: > 
 Instructive though it may be to code backpropagation from scratch, with torch, there is hardly ever a reason to. Its automatic differentiation feature, called autograd, keeps track of operations that need their gradients computed, as well as how to compute them.
...
are successively being *propagated* *back* through the network. This is
a form of *reverse mode automatic differentiation*.
...
not the input data [^1]. If subsequently we perform some operation on
...
To save memory, by default intermediate gradients are not stored.
Calling `retain_grad()` on a tensor allows to deviate from this default;
let's do this here, for the sake of demonstration:
...
to explicitly name the intermediate tensors any more. Just for fun and
to make the point, we can code forward pass, loss calculation and
backward pass in just three lines:
