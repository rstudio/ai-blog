...
Here are the transformations to be performed on the training set. Note how most of them are for data augmentation, while normalization is done to comply with what's expected by ResNet.
...
On the validation set, we don't want to introduce noise, while still needing to resize, crop, and normalize the images. The test set should be treated identically.
...
In the `train`, `valid`, and `test` directories, different classes of images reside in their own folder each. For example, here is the directory layout for the first three classes in the test set:
...
Now that we got the data, let us see how many items there are in each set.
...
Classes are coded as integers, to be used as indexes into a vector of class names. We'll use those for labeling the images.
...
But we want to distinguish between our 225 bird species, while ResNet was trained on 1000 different classes. What can we do? We simply replace the output layer.
...
This value is nothing but an anchor, however. *Learning rate schedulers* allow to evolve learning rates according to some proven algorithm. Among others, `torch` implements one-cycle learning [@abs-1708-07120], cyclical learning rates [@Smith15a], and cosine annealing with warm restarts [@LoshchilovH16a].
