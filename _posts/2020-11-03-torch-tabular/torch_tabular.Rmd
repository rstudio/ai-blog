...
What's more, if you're interested in deep learning (DL) particularly, and looking for the extra benefits to be gotten from big data, big architectures, and big compute, you're much more likely to build an impressive showcase on the former instead of the latter.
...
-   How to extract the continuous-space representations from the embedding modules.
...
Concretely, mushrooms are classified into two groups: edible and non-edible. The dataset description lists five possible rules with their resulting accuracies. While the least we want to go into here is the hotly debated topic of whether DL is suited to, or how it could be made more suited to, rule learning, we'll allow ourselves some curiosity and check out what happens if we successively remove all columns used to construct those five rules.
...
In `torch`, `dataset()` creates an R6 class. Like with most R6 classes, there will usually be a need for an `initialize()` method. Below, we use `initialize()` to preprocess the data and store it in convenient pieces. More on that in a minute. Prior to that, please note the two other methods a `dataset` has to implement:
...
In our example, both methods are straightforward to implement. `.getitem(i)` just directly uses its argument to index into the data, and `.length()` just returns the number of observations:
...
Our custom `dataset` defined, we create instances for training and validation each; and each gets their companion `dataloader`:
