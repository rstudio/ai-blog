...
a large number of columns within an R dataframe, and it is impractical to individually
enumerate all columns that need to be summed up. For example, here we have a six-column
dataframe of random real numbers, where the `partial_sum` column in the result
contains the sum of columns `b` through `d` within each row:
...
`sparklyr` 1.5 now also offers limited support for a column subsetting operator 
on Spark dataframes.
For example, all code snippets below will return some subset of columns from
the above dataframe named `sdf`:
...
Like that, it can accommodate users who want to use `sparklyr` but who do not necessarily have the required C++ compiler tool chain and
...
It is described in some detail in chapter 1, section 8 of [this document](https://cran.r-project.org/doc/manuals/r-patched/R-ints.pdf).
...
Also worth noticing is the fact that when an R dataframe containing only data types that
...
efficiently deserialized in R, rather than being delivered in other less efficient
formats.
