...
The features that we will discuss in details are:

- Initial support for JIT tracing
- Multiworker dataloaders
...
This feature is powered by the powerful {callr} package 
and works in all OS's supported by {torch}. {callr} let's
us create persistent R sessions thus, we only pay only once, the overhead of transferring potentially large datasets
objects to workers.

In the process of implementing this feature we have made 
dataloaders behave like {coro} iterators. This means that  
you can now use {coro}'s syntax to loop trough the dataloaders:
...
This also works for custom modules, that might also include sub-modules, for example:
...
my_module()
```
We hope that it makes easier to navigate understand the  `nn_module` objects. 
...
{torchaudio} is an extension for {torch} developed by [Athos Damiani](https://github.com/athospd) (@athospd), providing audio loading, transformations, common architectures for signal processing, pre-trained weights and access to commonly used datasets. An almost literal translation from PyTorch’s Torchaudio library to R.
...
- a Dcokerfile with GPU support as well as an [installation guide](https://github.com/mlverse/torch/blob/master/docker/build_env_guide.md) by @y-vectorfield 
