...
description: > 
  This article attempts to translate the [Daniel Falbel's](https://github.com/dfalbel) ['Simple Audio Classification'](https://blogs.rstudio.com/ai/posts/2018-06-06-simple-audio-classification-keras/) article from `{tensorflow}+{keras}` to `{torch}+{torchaudio}`.
author:
  - name: Athos Damiani
    affiliation: Curso-R
...
ds_train <- dataloader(
  ...
  collate_fn = collate_fn,
  num_workers = 8,
  worker_init_fn = function(x){library(torchaudio);set_audio_backend("tuneR_loader")},
...
{torchaudio} comes with 3 loaders, av_loader, tuner_loader, and audiofile_loader - more to come. The `set_audio_backend()` is used to set one of them as the audio loader. Their performances differs from audio format (mp3 or wav). There is no perfect world yet: tuner_loader is the best for mp3, audiofile_loader is the best for wav, but both do not have de option of partial load a sample from audio file without bringing all the data to memory first.
