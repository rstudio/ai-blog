...
description: > 
 We continue our exploration of time series forecasting with torch, moving on to architectures designed for multi-step prediction. In this first attempt, we augment the "workhorse RNN" by a multi-layer perceptron (MLP) meant to extrapolate multiple timesteps into the future. 
author:
  - name: Sigrid Keydana
    affiliation: RStudio
...
Apart from the fact that `.getitem()` now returns a vector for `y` as well as `x`, there is not much to be said about dataset creation. Without further ado, then, here is the complete code to set up the data input pipeline:
