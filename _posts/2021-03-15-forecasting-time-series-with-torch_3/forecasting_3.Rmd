...
Again, the dataset definition in the current post looks a bit different from the way it did before; again, it's the shape of the target that differs. This time, `y` equals `x`, shifted to the left by one.
...
The encoder takes its input and runs it through an RNN. Of the two things returned by a recurrent neural network, outputs and state, so far we've only been using the former. This time, we do the opposite: We throw away the outputs, and only return the state.

If the RNN in question is a GRU (and assuming that of the outputs, we take just the final time step, which is what we've been doing throughout), there really is no difference: The final state equals the final output. If it's an LSTM, however, there is a second, the "cell", state. In that case, returning the state instead of the final output will carry more information.
...
In the decoder, too, the main component is an RNN. In contrast to previously-shown architectures, though, it does not just return a prediction. It also reports back the RNN's final state.
...
In `seq2seq` is where the action happens. The plan is to encode once, then call the decoder in a loop.

If you look back to decoder `forward()`, you see that it takes two arguments: `x` and `state`.

Dependent on the context, `x` corresponds to one of three things.
...
`state`, too, is polyvalent. But here, there are just two possibilities.
...
Comparing this to the forecast obtained from last time's RNN-MLP combo, we don't see much of a difference. Is this surprising? To me it is. If asked to speculate about the reason, I would probably say that, likely, in all of the setups we've used so far, the main carrier of information has been the final hidden state of the RNN (the single RNN in the two previous architectures, the *encoder* RNN in this one). It will be interesting to check this hypothesis in the last part of this series, where we augment the encoder-decoder architecture by *attention*.
