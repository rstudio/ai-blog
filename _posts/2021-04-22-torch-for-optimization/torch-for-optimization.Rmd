...
Even though we ran the algorithm for three iterations, the optimal value really is reached after two. Seeing how well this worked, we try L-BFGS on a slightly more complex function, named *flower*, for pretty self-evident reasons.

## (Yet) more fun with L-BFGS

Here is the *flower* function.[^2] Mathematically, its minimum is near `(0,0)`, though the function itself is technically undefined at  `(0,0)`, since the `atan2` used in the function is not defined in that location.
...
For this reason, L-BFGS implementations commonly incorporate _line search_, that is, a set of rules indicating whether a proposed step length is a good one, or should be improved upon.
