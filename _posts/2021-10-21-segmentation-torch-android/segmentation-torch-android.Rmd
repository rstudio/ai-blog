...
3.  It [JIT-traces](https://blogs.rstudio.com/ai/posts/2021-08-10-jit-trace-module/) the trained model and saves it for deployment on mobile devices.
...
```{r}
# need torch > 0.6.1
# may have to run remotes::install_github("mlverse/torch") depending on when you read this
library(torch) 
library(torchvision)
...
All we have to do now is create a custom function that lets us decide on what augmentation to apply once per input-target pair, and then, manually call the respective transformation functions.
...
Now on to running this model "in the wild" (well, sort of).
...
<aside>Please see our [introduction to the torch JIT compiler](https://blogs.rstudio.com/ai/posts/2021-08-10-jit-trace-module/).</aside>
...
However, since we already know we'd like to deploy it on Android, we directly make use of the specialized function `jit_save_for_mobile()` that, additionally, generates bytecode:
