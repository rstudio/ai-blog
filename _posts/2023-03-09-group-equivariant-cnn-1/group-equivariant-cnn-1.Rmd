---
title: "An upside-down cat is still a cat: Evolving image recognition with Geometric Deep Learning"
description: >
  In this first in a series of posts on group-equivariant convolutional neural networks (GCNNs), meet the main actors -- groups -- and concepts (equivariance). With GCNNs, after a break far too long, we finally revisit the topic of Geometric Deep Learning, a principled, math-driven approach to neural networks that has consistently been rising in scope and impact.
author:
  - name: Sigrid Keydana
    affiliation: Posit
...
-   By domain, I'm referring to the underlying physical space, and the way it is represented in the input data. For example, images are usually coded as a two-dimensional grid, with values indicating pixel intensities. (At least, that's when the image has a single channel. Otherwise, there are a number of grids, each mapped to a separate channel.)
-   The task is what we're training the network to do: classification, say, or segmentation. (More accurately, tasks may be different at different stages in the architecture. At each stage, the task in question will have its word to say about how layer design should look.)
...
In a nutshell, we were able to specify design desiderates based on (1) what we're given and (2) what we're tasked with.
...
Following these examples, let me generalize. Here is typical definition. [^1]

[^1]: From [Wolfram Alpha](https://mathworld.wolfram.com/Group.html) (beginning of article).
...
That's it for today. Hopefully, I've managed to illuminate a bit of *why* we would want to have group-equivariant neural networks. The question remains: How do we get them? This is what the remaining posts in the series will be about.
